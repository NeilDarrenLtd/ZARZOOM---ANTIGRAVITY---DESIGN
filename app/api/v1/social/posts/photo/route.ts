...
import { photoPostSchema } from "@/lib/social";
 ...
 *
 * Input: { profile_username, platforms[], text, image_asset_id | image_url, schedule_at?, callback_url? }
...
export const POST = createApiHandler({
  ...
  handler: async (ctx) => {
    ...
    const input: PublishMediaInput = {
      ...parsed.data,
      media_asset_id: parsed.data.image_asset_id,
      media_url: parsed.data.image_url,
    };

    return publishPost(ctx, "photo", input);
  },
});
