 ...
 * Returns the job status and, if complete, the article or script artefact.
 */
export const GET = createApiHandler({
  ...
  handler: async (ctx) => {
    ...
    const rawJobId = segments[segments.indexOf("writing") + 1] ?? "";
    ...
    if (!job) {
      throw new NotFoundError("Job", `Writing job "${job_id}" not found`);
    }
