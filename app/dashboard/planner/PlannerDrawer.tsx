 ...
 * PlannerDrawer
 *
 * Slide-over editor panel for a single PlannerItem.
 * Opens from the right when an item is selected on the calendar.
 *
 * Save / Cancel / Delete are handled locally against the usePlannerItems hook.
 * TODO (update planner item): Wire onSave to usePlannerItems.updateItem().
 * TODO (delete planner item): Wire onDelete to usePlannerItems.deleteItem().
 */
...
interface PlannerDrawerProps {
  ...
  onClose: () => void;
  /** Called with the updated item when Save is confirmed */
  onSave?: (updated: PlannerItem) => void;
  /** Called with the item id when Delete is confirmed */
...
export default function PlannerDrawer({ item, onClose, onSave, onDelete }: PlannerDrawerProps) {
  ...
  function handleSave() {
    if (form) {
      // TODO (update planner item): Replace with usePlannerItems.updateItem(form.id, form)
      onSave?.(form);
      setIsDirty(false);
    }
  }
  ...
  function handleDelete() {
    if (form) {
      // TODO (delete planner item): Replace with usePlannerItems.deleteItem(form.id)
      onDelete?.(form.id);
      onClose();
    }
  }
        ...
        {form ? (
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-5">
                  ...
                  <FieldGroup label="Viral Strength">
                    <div className="relative">
                      <select
                        ...
                        onChange={(e) =>
                          update("viralStrength", Number(e.target.value) as ViralStrength)
                        }
                        className={selectBase}
                      >
                        {([1, 2, 3, 4, 5] as ViralStrength[]).map((v) => (
                          <option key={v} value={v}>{VIRAL_STRENGTH_LABELS[v]}</option>
                        ))}
                      </select>
                      <ChevronDown className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                    </div>
            ...
            <div className="flex items-center gap-2 flex-wrap pt-1">
              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${TYPE_COLORS[form.type]}`}>
                {TYPE_LABELS[form.type]}
              </span>
              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[form.status]}`}>
